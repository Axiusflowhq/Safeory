...
pub fn collect_deadlines(items: &[VaultItem], today: (i32, u32, u32)) -> Vec<Deadline> {
    ...
    for item in items {
        if let Some(deadline) = deadline_for_item_with_today_days(item, today_days) {
...
/// Derive the single deadline represented by an active item, if any.
///
/// This is the streaming form of collect_deadlines: callers that decrypt
/// records one-at-a-time can project reminder metadata without retaining the
/// full decrypted vault in memory.
pub fn deadline_for_item(item: &VaultItem, today: (i32, u32, u32)) -> Option<Deadline> {
    deadline_for_item_with_today_days(item, days_since_civil(today.0, today.1, today.2))
}

fn deadline_for_item_with_today_days(item: &VaultItem, today_days: i64) -> Option<Deadline> {
    let (field, label): (&str, &'static str) = match item.kind {
        ...
        ItemKind::Receipt => {
            ...
            match status {
                ...
                "refunded" | "kept" | "returned" => return None,
    ...
    let date_str = item.fields.get(field).map_or("", String::as_str).trim();
