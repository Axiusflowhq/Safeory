...
- Encrypted backup creation and restore are backend-owned. Native Rust dialogs choose backup destinations, so the renderer neither supplies nor receives the saved path. After the validated staged snapshot has been successfully published, Safeory attempts to record a device-local creation timestamp; failure of that secondary settings write leaves the published backup intact and the previous recorded timestamp unchanged. The activity record never contains a destination path or file identity and does not assert that the external file still exists or reflects later vault changes. Restore accepts the selected source path and credential, validates a staged copy including SQLite integrity and authenticated decryption of every persisted lifecycle state, then transactionally copies encrypted tables into the live database. Restore preserves the wrapped root key and encrypted record bytes/revisions rather than decrypting and reserializing the vault, and it does not import or replace device-local backup-activity metadata.
