...
- The local Plan Test is deliberately narrower than future emergency release. Its readiness response contains only booleans about recovery configuration and Emergency Card completeness; it does not return contact details, instructions, or selected record identities. Recovery-key self-test accepts the candidate only while the vault is already unlocked, zeroizes the Rust input, compares the recovered root to the authoritative in-memory root inside the crypto boundary, returns only match/mismatch, and does not replace the session or persist a verification flag. It does not simulate trusted-person sharing, waiting periods, timed release, or destruction.
- Human-readable JSON export and encrypted database backup are explicit user actions that write outside the app-data directory. The JSON export is plaintext by design, contains active record data only, excludes Trash/tombstones, and does not inline binary attachment contents; the UI labels that boundary. Encrypted backups use SQLite's snapshot API, are reopened and validated before publication, and preserve Trash, item and attachment tombstones, recovery configuration, object IDs/revisions, encrypted attachment manifests, and all attachment chunks.
