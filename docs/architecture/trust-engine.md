...
2. ADR 0006 defines the high-entropy Account Secret, remote root envelope,
   local-format migration, and recovery invariants. The Rust/WASM foundation and
   bounded revision-fenced sync object are implemented; application publication
   UX, enrollment/recovery coordination, signed device transfer, and external
   review remain. Cognito authentication does not replace client-side key
   protection.
